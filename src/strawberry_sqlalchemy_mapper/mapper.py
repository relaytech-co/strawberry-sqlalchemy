...
_PropertNameMapperDescriptor = Union[
...
class StrawberrySQLAlchemyMapper(Generic[BaseModelType]):
    ...
    #: Function to get the strawberry property name for a given SQLAlchemy property
    #: (sql_alchemy_property_name, sql_alchemy_descriptor) -> strawberry_property_name
    property_name_mapper: Callable[
        [str, _PropertNameMapperDescriptor],
        str,
    ]
    ...
    def __init__(
        ...
        property_name_mapper: Optional[
            Callable[
                [
                    str,
                    _PropertNameMapperDescriptor,
                ],
                str,
            ]
        ] = None,
        ...
        if property_name_mapper is None:
            property_name_mapper = self._default_property_name_mapper
        self.property_name_mapper = property_name_mapper
        self.edge_types = {}
    ...
    @staticmethod
    def _default_property_name_mapper(
        name: str,
        descriptor: _PropertNameMapperDescriptor,
    ) -> str:
        return name
    ...
    def _handle_columns(
        ...
        for key, column in mapper.columns.items():
            ...
            if type_annotation is not SkipTypeSentinel:
                property_name = self.property_name_mapper(key, column)
    ...
    def type(
        ...
        def convert(type_: Any) -> Any:
            ...
            for key, relationship in mapper.relationships.items():
                ...
                property_name = self.property_name_mapper(key, relationship)
                self._add_annotation(
                    type_,
                    property_name,
                    strawberry_type,
                ...
                if key in model.__annotations__:
                    ...
                    for (  # type: ignore[assignment]
                        ...
                        if isinstance(annotation, sqlalchemy_type):
                            property_name = self.property_name_mapper(key, descriptor)
                            self._add_annotation(
                                type_,
                                property_name,
                                strawberry_type,
                    ...
                    property_name = self.property_name_mapper(key, descriptor)
                    self._add_annotation(
                        type_, property_name, strawberry_type, generated_field_keys
                    )
                    sqlalchemy_field = cast(
                        StrawberryField,
                        field(
                    ...
                    assert not sqlalchemy_field.init
                    setattr(type_, property_name, sqlalchemy_field)
